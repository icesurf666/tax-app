...
const CityTable: React.FC = () => {
  ...
  const body = city.cities.map((row) => (
  ...
  const onDelete = (values: ICity) => {
    city.removeCity(values.id)
    setActiveModal(false)
  }

  const onEdit = (values: ICity) => {
    city.updateCity(values)
    setActiveModal(false)
  }

  const onAdd = (values: ICity) => {
    city.addCity({id: uuid(), name: values.name})
    setAddActive(false)
  }
